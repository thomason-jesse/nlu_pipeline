...
class LinearLearner:
	...
	def updateThetaMapReturnFullFeatureMap(self, m):
		...
		self.copyDict(full_feature_map, self.theta) #restore theta values at m locations; theta now expanded with 0s at new features
	...
	def zeroDict(self, d):
		for k in d:
			if (type(d[k]) is not dict): d[k] = 0
			else: self.zeroDict(d[k])
	def copyDict(self, s, t):
		for k in s:
			if (type(s[k]) is not dict): t[k] = s[k]
			else:
				if (k not in t): t[k] = {}
	...
	def scoreFeatures(self, m, lt=None):
		...
		s = 0
		for k in m:
			if (type(m[k]) is not dict): s += m[k]*lt[k]
			else: s += self.scoreFeatures(m[k],lt[k])
		return s
	...
	def updateTheta(self, f, g, lr, lt=None):
		...
		for k in lt:
			if (type(lt[k]) is not dict): lt[k] += lr*(g[k]-f[k])
			else: self.updateTheta(f[k],g[k],lr,lt[k])
